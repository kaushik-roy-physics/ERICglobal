...
def generate_psi_diff(theta, omega, K, L, N, dt, tsteps):
    ...
    for i in range(nontransient_steps):
        theta += dtheta_dt(theta, omega, K, L, N) * dt
...
dt = 0.1  # Timestep
tsteps = int(T / dt) + 1  # total number of steps

    
# Set the coupling strength
K = 4.0  # fixed K value
L_values = cp.linspace(0.0, 10.0, 200)  # range of L values
...
output_filename = "psidiffvsLK4tdisc.json"
