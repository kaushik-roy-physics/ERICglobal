...
K = cp.linspace(2, 4, 100)
K_ratios = K / 2  # Calculate K_ratios
K_ratios_cpu = K_ratios.get()  # To use for plotting
L_value = 0.0  # Use a single L value
...
H_norm = cp.zeros(len(K_ratios))

for i in cp.arange(len(K_ratios)):
    H_norm[i] = compute_daido_norm(2 * K_ratios[i], L_value, N, T, dt, omega_in, theta_in)  # Double K_ratios
...
plt.plot(K_ratios_cpu, H_norm_values[0], label=f"$\Lambda$ = {L_value}", color='b')
