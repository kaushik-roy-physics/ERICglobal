...
def animate(i):
    global theta
    omega_avg = (1/N) * cp.sum(dtheta_dt(theta, omega_in, K, L, N))
    psi = theta - omega_avg * i * dt
    
    theta_osc = (psi + cp.pi) % (2 * cp.pi) - cp.pi
...
ani.save('phasesvstime_K4Lsmall_rotated.mp4', writer='ffmpeg', fps=10)
