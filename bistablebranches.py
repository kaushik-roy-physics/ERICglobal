...
#condition1 = cp.abs(omega_in  - (K * (H_daido_values - L_values/2))) < 1e-4
...
plt.scatter(all_theta, all_omega, s=10, label='All Oscillators', alpha=0.5)

# Overlay the satisfying omega_in vs final_theta in a different color
plt.scatter(theta_satisfying, omega_satisfying, s=12, color='red', label='Satisfying Oscillators')

plt.xlabel(r'$\theta_{i}$', fontsize = 20)
plt.ylabel(r'$\omega_{i}$', fontsize = 20)
plt.ylim(-10,4)
#plt.title('Omega_in vs. Final Theta')
plt.legend(loc = 'lower right', fontsize = 15)
#plt.grid(True)
plt.savefig("omegavstheta_bistable_fullt.pdf")
plt.show()
