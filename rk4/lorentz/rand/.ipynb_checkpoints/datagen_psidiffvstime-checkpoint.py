...
def rk4(theta,omega,K,L,N,dt):
    k1 = dt * dtheta_dt(theta, omega, K, L, N)
    k2 = dt * dtheta_dt(theta + 0.5 * k1, omega, K, L, N)
    k3 = dt * dtheta_dt(theta + 0.5 * k2, omega, K,  L, N)
    k4 = dt * dtheta_dt(theta + k3, omega, K, L, N)
...
def generate_psis(theta, omega, K, L, N, dt, tsteps):
    ...
    for i in range(tsteps):
        theta = rk4(theta,omega,K,L,N,dt)
...
#    L_values = [0, 2.5, 3.0]
    ...
    output_filename = "psidiffvstimeK4Lsmall.json"
