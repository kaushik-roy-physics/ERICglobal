...
def plot_data(K_cpu, rho_1_values, rho_2_values, D_values, filename_prefix):
    ...
    for i, D in enumerate(D_values):
        ax[0].plot(K_cpu, rho_1_values[i], linestyle = line_styles[i % 4] ,marker = markers[i % 4], ms = 5,  markevery = 5,  label=f'$D$ = {D}')
        ax[1].plot(K_cpu, rho_2_values[i], linestyle = line_styles[i % 4] ,marker = markers[i % 4], ms = 5, markevery = 5,  label=f'$D$ = {D}')

#    for i, D in enumerate(D_values):
#        ax[0].plot(K_cpu, rho_1_values[i], color=colors[i], label=f'$D$ = {D}')
#        ax[1].plot(K_cpu, rho_2_values[i], color=colors[i], label=f'$D$ = {D}')
...
filename = "rhovsKnearonsetLhalf_2.json"
...
plot_data(K_cpu, rho_1_values, rho_2_values, D_values, filename_prefix)
